...
public class ArithmeticUtil {
    ...
    ArrayList<PositionModel> listPosAll = new ArrayList<PositionModel>();

    ArrayList<UserModel> listUserAll = new ArrayList<UserModel>();

    //基于pv流行性推荐算法
    // map          ->  存在ServletContext中所有职位当日PV数
    // user         ->  当前用户
    // listPos      ->  所有职位 列表
    public ArrayList<PositionCompanyBO> popularityRec(HashMap<Integer, Integer> map, UserModel user) {
        ...
        for (PositionModel pos : listPosAll) {
            //定义该职位当日pv数
            int pv = 0;
...
//基于用户的协同过滤算法
    ...
    // user     ->  当前用户
    public ArrayList<PositionModel> synergyUserRec(UserModel user) {
        ...
        int count = 0;
        for (UserModel u : listUserAll) {
            //矩阵元素填充，用户收藏该职位，则元素值为1，否则值为0
            for (PositionModel p : listPosAll) {
    ...
    public ArrayList<PositionModel> recordRec(int hostId, TreeMap<Double, Integer> map) {
        ...
        //推荐职位 队列
        ArrayList<PositionModel> recList = null;
    ...
    //基于职位协同过滤算法
    //user  ->  当前用户
    public ArrayList<PositionCompanyBO> synergyItemRec(UserModel user) {
        ...
        for (int i = 0; i < listPosAll.size(); i++) {
            ...
            for (int j = 0; j < listUserAll.size(); j++) {
                ...
                CommentModel com = ariConst.commentMapper.listComment(userItemId, posItemId);
                if (com != null) {
                    switch (com.getType()) {
                        case 1:
                            itemArray[j] += 1;
